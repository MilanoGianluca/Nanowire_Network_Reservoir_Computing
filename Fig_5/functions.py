...
def update_edge_weigths_2(G,delta_t, Y_min_m, Y_min_s, Y_max_m, Y_max_s, kp0, eta_p_m, eta_p_s, kd0, eta_d_m, eta_d_s):

    np.random.seed(2)
    g_min_array = np.random.normal(Y_min_m, Y_min_s, G.number_of_edges())
    
    np.random.seed(3)
    g_max_array = np.random.normal(Y_max_m, Y_max_s, G.number_of_edges())
    
    np.random.seed(4)
    eta_p_array = np.random.normal(eta_p_m, eta_p_s, G.number_of_edges())
    
    np.random.seed(5)
    eta_d_array = np.random.normal(eta_d_m, eta_d_s, G.number_of_edges())
    

    edge_count = 0
    
    for u,v in G.edges():
        Y_min = g_min_array[edge_count]
        Y_max = g_max_array[edge_count]
        eta_p = eta_p_array[edge_count]
        eta_d = eta_d_array[edge_count]
        edge_count += 1
